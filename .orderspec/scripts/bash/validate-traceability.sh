  ...
  while IFS= read -r ln; do
    ...
    for p in $(extract_paths "$ln"); do
      if [ "$tag" = "[MOD]" ] && [ ! -e "$REPO_ROOT/$p" ]; then
        add M10 HIGH "plan.md" "[MOD] path '$p' does not exist in repo — stale plan, regenerate /order.plan"
      elif [ "$tag" = "[NEW]" ] && [ -e "$REPO_ROOT/$p" ]; then
        add M10 HIGH "plan.md" "[NEW] path '$p' already exists in repo — stale plan, regenerate /order.plan"
      fi
    done
  done <<< "$ANNOT_LINES"
fi
