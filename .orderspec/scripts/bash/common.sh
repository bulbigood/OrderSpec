...
# Find repository root by searching upward for .orderspec directory
# This is the primary marker for spec projects
find_orderspec_root() {
    local dir="${1:-$(pwd)}"
...
# Get repository root, prioritizing .orderspec directory
# This prevents using a parent repository when orderspec is initialized in a subdirectory
get_repo_root() {
    # First, look for .orderspec directory (orderspec's own marker)
    local orderspec_root
    if orderspec_root=$(find_orderspec_root); then
        echo "$orderspec_root"
        return
    fi
...
# the git extension) or implicitly via .orderspec/feature.json.
get_current_branch() {
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.orderspec/feature.json"
...
# Persist a feature_directory value to .orderspec/feature.json.
...
get_feature_paths() {
    ...
    #   2. .orderspec/feature.json "feature_directory" key (persisted by spec command)
    ...
    if [[ -n "${SPECIFY_FEATURE_DIRECTORY:-}" ]]; then
    ...
    elif [[ -f "$repo_root/.orderspec/feature.json" ]]; then
        local _fd
        ...
        if [[ -n "$_fd" ]]; then
            ...
            echo "ERROR: Feature directory not found. Set SPECIFY_FEATURE_DIRECTORY or ensure .orderspec/feature.json contains feature_directory." >&2
            return 1
        fi
    else
        echo "ERROR: Feature directory not found. Set SPECIFY_FEATURE_DIRECTORY or run the spec command to create .orderspec/feature.json." >&2
        return 1
    fi
...
get_invoke_separator() {
    ...
    local integration_json="$repo_root/.orderspec/integration.json"
    local separator="."
...
format_order_command() {
    ...
    command_name="${command_name#order.}"
    command_name="${command_name#order-}"
...
#   2. .orderspec/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .orderspec/extensions/<ext-id>/templates/
#   4. .orderspec/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.orderspec/templates"
    ...
    local presets_dir="$repo_root/.orderspec/presets"
    if [ -d "$presets_dir" ]; then
        ...
        if [ -f "$registry_file" ] && command -v python3 >/dev/null 2>&1; then
            ...
            if sorted_presets=$(ORDERSPEC_REGISTRY="$registry_file" python3 -c "
import json, sys, os
try:
    with open(os.environ['ORDERSPEC_REGISTRY']) as f:
        data = json.load(f)
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.orderspec/extensions"
    if [ -d "$ext_dir" ]; then
                ...
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        ...
                        if [ -f "$manifest" ] && command -v python3 >/dev/null 2>&1; then
                            ...
                            local py_stderr
                            py_stderr=$(mktemp)
                            result=$(ORDERSPEC_MANIFEST="$manifest" ORDERSPEC_TMPL="$template_name" python3 -c "
import sys, os
try:
    import yaml
except ImportError:
    ...
    sys.exit(0)
try:
    with open(os.environ['ORDERSPEC_MANIFEST']) as f:
        data = yaml.safe_load(f)
    for t in data.get('provides', {}).get('templates', []):
        if t.get('name') == os.environ['ORDERSPEC_TMPL'] and t.get('type', 'template') == 'template':
