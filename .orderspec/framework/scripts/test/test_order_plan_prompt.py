...
class TestOrderPlanPrompt(unittest.TestCase):
    ...
    def test_work_order_baseline_and_cross_boundary_contract(self):
        ...
        self.assertIn("plan_reconcile.py classify-impact", plan_prompt)
        self.assertIn("candidate-template", plan_prompt)
        self.assertIn("`changed_spec_ids`", plan_prompt)
        self.assertIn("`evidence_dependency_spec_ids`", plan_prompt)
        self.assertIn("they never imply\n   completed-work overlap", plan_prompt)
        self.assertIn("user's\n   configured language", plan_prompt)
        self.assertIn("choice label and consequence", plan_prompt)
        self.assertIn("PENDING_ONLY", plan_prompt)
        self.assertIn("OPERATOR_PRESERVE_APPROVED", plan_prompt)
        self.assertIn("Never hand-author `OPERATOR_INPUT`", plan_prompt)
        self.assertIn("separate `MUTATION_APPROVAL`", plan_prompt)
        self.assertIn("task ownership proves the transition", plan_prompt)
        self.assertIn("missing/mismatched baseline", plan_prompt)
