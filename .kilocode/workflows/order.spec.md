...
- If `.orderspec/extensions.yml` exists, read entries under `hooks.before_spec`. If missing or unparsable YAML, skip silently.
...
5. **Re-size check**: if the refinement has pushed the contract past the decomposition threshold (§6 heuristic), surface a one-line note recommending `/order.spec --split` — do not auto-split mid-refine.
...
a. Create or update `SPECIFY_FEATURE_DIRECTORY/checklists/requirements.md` (in refine mode, update existing rather than recreate):
   ...
   **Feature**: [Link to spec.md]
   ...
   ## Notes
   - Items marked incomplete require spec updates before `/order.plan`
   ```
...
- If `.orderspec/extensions.yml` is missing, unparsable, or has no `hooks.after_spec` entries, skip to Completion Report.
...
- **create**: `SPECIFY_FEATURE_DIRECTORY`, `SPEC_FILE`, checklist results, ID counts (REQ/UJ/AC/INV/EDGE), readiness for `/order.plan`.
