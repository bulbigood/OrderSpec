...
- If `.orderspec/extensions.yml` exists, read entries under `hooks.before_spec_check`. If missing or unparsable YAML, skip silently.
...
- If `.orderspec/extensions.yml` is missing, unparsable, or has no `hooks.after_spec_check` entries, skip silently.
