...
2. **Repo-staleness** — plan's `[NEW]`/`[MOD]` paths no longer match the actual repository *now*, just before code.
...
- You are about to run an **expensive `/order.code`** and want a fresh-context, whole-system review.
...
- If `.orderspec/extensions.yml` exists, read entries under `hooks.before_analyze`. If missing or unparsable YAML, skip silently.
...
- Requirement quality / ambiguity / vague adjectives, spec self-consistency, glossary, AC form, spec purity → `/order.spec` + `spec-check`.
- plan↔spec completeness, mechanism adequacy, plan role-purity, plan not duplicating spec, stack consistency → `/order.plan` + `plan-check`.
- tasks E-M-C ordering, coverage self-validation, SC→task reflection, test-first, task format/numbering → `/order.tasks` + `tasks-check`.
...
**Run**: `/order.{spec|plan|tasks} "{ready-to-run request}"`  (for a cascade, list the ordered sequence of commands)
```
...
- S2b: Use the script's M8/M10 for path existence and `[NEW]`/`[MOD]` correctness. Add judgment only where the script cannot: a `[MOD]` file exists but plan assumes a function/export now absent. Sample at most 2 high-risk claims. A vanished target → **Route** (→ `/order.plan`).
...
### Routing Required (owned by /order.spec / .plan / .tasks — gate did NOT modify content)
...
- **Cascade** (root in spec → regenerate plan+tasks) → presented as an ordered Routing sequence (`/order.spec "..."` → `/order.plan` → `/order.tasks`); the architect executes it. The gate never deletes or regenerates anything.

Route remediation to the cheapest artifact consistent with the likely source of truth. Recommended loop: run the routed commands, then **re-run `/order.analyze`** to confirm the system is consistent. Idempotent: a consistent system yields ✅ PASS for `/order.code`.

### 7. Post-Execution Hooks

- If `.orderspec/extensions.yml` is missing, unparsable, or has no `hooks.after_analyze` entries, skip silently.
