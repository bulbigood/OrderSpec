...
This is a **per-stage gate** for one document: `spec.md`. It runs after `/speckit.specify` and answers a single question:
...
- **Authoring contract content is owned by `/speckit.specify`**, not by this gate. If the user wants to add, change, or fill in spec content, they invoke `/speckit.specify "<what they want>"`. That command owns the elicitation taxonomy, ID-routing, and write discipline.
- **This gate is a pure inspector.** It verifies the contract and may perform only **mechanical / meaning-preserving** auto-fixes (glossary spelling, numbering, broken internal references, format). Anything that would **change requirement meaning, scope, thresholds, or fill a missing topic** is NOT done here — it is surfaced as a **Routing block** telling the user exactly which `/speckit.specify` call will resolve it.

So the gate has two output channels only:
1. **Auto-Fixed** — mechanical defects it silently corrected in place.
2. **Routing Required** — contractual findings it cannot touch, each with a ready-to-run `/speckit.specify` invocation.
...
- authoring/filling spec content, elicitation, ID-routing, **and decomposition (`--split`)** → `/speckit.specify`.
- plan↔spec completeness, mechanism adequacy, stack → `plan-check`.
- tasks ordering / coverage-by-task → `tasks-check`.
- repo state, drift, whole-system constitution sweep, cross-artifact contradictions → `/speckit.analyze`.
- mechanical ID inventory, dangling internal references, numbering → the script.

## When to Run

Conditional. Recommended after every `/speckit.specify` run (including refinement runs), especially when:
...
- You want a clean, fully-covered root before `/speckit.plan` consumes it.

Can be wired as an automatic post-specify hook (`hooks.after_specify`) for weaker-model workflows.

## Pre-Execution Checks

**Check for extension hooks (before validation)**:
- If `.specify/extensions.yml` exists, read entries under `hooks.before_spec_check`. If missing or unparsable YAML, skip silently.
...
Fix only strictly mechanical / meaning-preserving defects. Route everything contractual to `/speckit.specify`.

## Out of Scope (do NOT do here)

- **Authoring or filling spec content** — adding REQ/NFR/CON/INV/EDGE/AC, defining terms, setting thresholds, resolving `[NEEDS CLARIFICATION]` by writing an answer. All of this is `/speckit.specify`'s job; the gate only routes to it.
- **Performing the decomposition** — splitting an oversized spec into sub-specs is a content/authoring action owned by `/speckit.specify --split`; the gate only detects oversized scope and routes.
- How the spec will be implemented — mechanisms, stack, paths → `plan-check` / `/speckit.plan`.
- Topics that are genuinely planning-time decisions (which library, which deployment, code structure) → tag the finding **"defer-to-plan"** (do NOT route to specify; it carries forward as a known planning decision).
- Whether tasks cover the spec → `tasks-check`.
- Repository state, drift, whole-system constitution sweep, cross-artifact contradictions → `/speckit.analyze`.
...
- **Auto-fix vs Route boundary** (the single permission to write, kept deliberately tight):
  ...
  - **Route** for **everything else** — anything touching requirement meaning, threshold, scope, term definition, EDGE behavior, or a missing topic. Emit a **Routing block** naming the defect and the exact `/speckit.specify` call that will resolve it. The gate does NOT write the resolution and does NOT wait to apply one.
  - **When in doubt, Route — never Auto-fix.** The write permission is the narrow exception, not the norm.
- **No Ask-and-apply, no Decision-blocks-that-mutate**: the gate never asks the user a question whose answer it then writes into the spec. Content decisions are made by the user *through* `/speckit.specify`. The gate's job ends at producing precise Routing blocks.
- **Constitution authority**: a spec *goal* directly conflicting with a `.specify/memory/constitution.md` MUST principle is CRITICAL → Route (the principle is fixed; reconciling the goal is a content change owned by specify). The full whole-system constitution sweep remains `/speckit.analyze`'s job.
...
**Run**: `/speckit.specify "{ready-to-run refinement request capturing the finding}"`
```
...
Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` once from repo root; parse FEATURE_DIR and FEATURE_SPEC. Derive: SPEC = FEATURE_SPEC. Locate `FEATURE_DIR/checklists/requirements.md` if present (read-only). plan.md and tasks.md may be absent and are NOT read by this gate. If EXISTS, load `.specify/memory/constitution.md` for governance constraints. Abort with instructions (re-run `/speckit.specify`) if SPEC is missing. For single quotes in args use `'I'\''m Groot'` or double quotes.
...
- Any risky `ASM-NNN` default in §17 whose wrongness would change behavior → Route (confirm or revise via specify).
...
- C1a: Each REQ is **verifiable** — a tester could devise a pass/fail check. Unobservable/unmeasurable → **Route** (specify must add the observable criterion/threshold).
- C1b: Quantitative behavior ("fast","shortly") has a **measurable threshold**. Vague adjective without a number → **Route** (specify must set the contract value — the gate never picks a number).
...
- C2a: No two REQs contradict; no REQ contradicts an INV or CON. Genuine contradiction → **Route** (specify must decide which governs; the gate never silently deletes one).
...
- C4a: Each user-journey step (§14/§15) is backed by ≥1 REQ/AC. Unbacked step → **Route** (specify must add the backing requirement).
- C4b: Each named EDGE case has **defined expected behavior**. Listed without resolution → **Route** (specify must define the behavior).
...
- C6b: Each SC is an **outcome**, not a task ("users complete checkout in <3 steps", not "add a checkout button"). SC-as-task → **Route** (specify must rephrase to the intended outcome).

#### C7. Direct Constitution Conflicts (narrow)

- C7a: Flag only a **direct** spec-goal-vs-constitution conflict you encounter. Any conflict is CRITICAL → **Route** (the principle is fixed; reconciling the goal is a content change owned by specify). Do NOT perform the full constitution sweep — that is `/speckit.analyze`.

#### C8. Scope Sizing (detect oversized contract — route to decomposition)

The contract should be **cohesive enough to plan and build as one unit**. A spec spanning many independent domains is expensive to plan, hard for a weaker model to keep consistent, and tends to hide cross-domain contradictions. Detect oversize; **never split here** — decomposition authors new artifacts and is owned by `/speckit.specify --split`.
...
- C8a: Estimate breadth from the current spec: distinct functional domains, independent primary actor sets with non-overlapping journeys, REQ volume, and the number of viable P1-MVP threads.
  ...
  - **Oversized** → **Route** to `/speckit.specify --split`. In *Suggested direction*, list the natural module boundaries you observed (advisory only — the gate proposes seams, the author decides and decomposes). Severity MEDIUM (HIGH only if the breadth already produced a cross-domain contradiction you flagged under C2).
...
- Re-evaluate each item against the current spec. **Do NOT modify the file** — toggling checkboxes follows content changes, and content is owned by `/speckit.specify`.
...
### Routing Required (owned by /speckit.specify — gate did NOT modify content)
...
- **Contractual (meaning / threshold / scope / missing topic)** → see the Routing block; the user resolves it by running the suggested `/speckit.specify "..."`. The gate did not and will not write this content.
- **defer-to-plan** → noted, not routed to specify; it carries forward as a known planning decision for `/speckit.plan`.
- **Oversized scope (C8)** → see the Routing block; the user decomposes via `/speckit.specify --split`, which proposes module seams, builds one focused sub-spec on confirmation, and emits ready-to-run prompts for the rest. The gate never splits.

Recommended loop: run the routed `/speckit.specify` calls (batch the requests if convenient), then **re-run `/speckit.spec-check`** to confirm the contract is now clean. The gate is idempotent: a spec with no contractual findings yields ✅ PASS.

Downstream note: if plan/tasks already exist and a routed specify change lands, they may need `/speckit.plan` / `/speckit.tasks` regeneration — but that is observed by `plan-check` / `tasks-check` / `/speckit.analyze`, not acted on here.

### 8. Post-Execution Hooks

- If `.specify/extensions.yml` is missing, unparsable, or has no `hooks.after_spec_check` entries, skip silently.
...
- **Detect oversize, never decompose**: C8 flags a contract too broad for one cohesive spec and routes to `/speckit.specify --split`; proposing seams is advisory, authoring sub-specs is the owner's job. Density alone is never a defect.
- **Pure inspector for content**: the gate detects and routes; it NEVER authors, fills, or alters contract meaning. Filling spec content is `/speckit.specify`'s job, invoked by the user.
...
- **Route, don't ask-and-apply**: contractual findings become Routing blocks with a ready-to-run `/speckit.specify "..."` — the gate never asks a content question whose answer it would write.
- **Detect missing topics, but never fill them**: the C0 taxonomy sweep finds gaps and routes them; authoring the missing REQ/NFR/EDGE belongs to specify.
- **Root stance**: there is no upstream to reroute to — the only "upstream" is the user (through specify). Planning-time gaps are tagged "defer-to-plan", never routed to specify. Never read repo, plan, or reason about time.
- **Read-only on the checklist**: cross-check `requirements.md` against the spec and report disagreements; never toggle it (markers follow content, which specify owns).
